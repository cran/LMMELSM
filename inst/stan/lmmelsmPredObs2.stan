...
parameters {
  ...
  row_vector[J] sigma; // *Log*-scale sigma
...
transformed parameters {
  ...
  eta += rep_matrix(nu, N); // eta += intercepts
  if(P >= 1) { // + XB
...
model {
  ...
  if(!prior_only){
	...
	epsilon_cov_L[k] = (diag_pre_multiply(exp(eta_logsd[l1_indices[k]]), epsilon_L));
      ...
      for(n in 1:N) {
	y[n,] ~ multi_normal_cholesky(eta[n], epsilon_cov_L[group[n]]);
      }
    } else {
	for(n in 1:N) {
	    y[n,] ~ multi_normal_cholesky(eta[n,], diag_pre_multiply(exp(eta_logsd[n,]), epsilon_L));
	}
    }
  }
}
